...
module Instructeurs
  class DossiersController < ProceduresController
    ...
    def suivi_et_decision
      @following_instructeurs_emails = dossier.followers_instructeurs
        .joins(:follows).merge(Follow.order(id: :desc)).map(&:email)
      previous_followers = dossier.previous_followers_instructeurs - dossier.followers_instructeurs
      @previous_following_instructeurs_emails = previous_followers.map(&:email)
      @avis_emails = dossier.experts.map(&:email)
      @invites_emails = dossier.invites.map(&:email)
      @potential_recipients = dossier.groupe_instructeur.instructeurs
        .reject { |g| g == current_instructeur }
      @assignments = dossier.dossier_assignments
        .includes(:groupe_instructeur, :previous_groupe_instructeur)
        .order(:assigned_at)

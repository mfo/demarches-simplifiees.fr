...
- prefer `.each { it }` instead of `.each { _1 }`
- you can user shorthand assignation like `{procedure_revision: } #when a local var exists`
- only add comment for complex stuff, not needed for classic stuff
